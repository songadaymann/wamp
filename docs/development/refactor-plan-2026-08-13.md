...
| T10 | World-streaming model, selection policy, and readiness | Accepted: manual `PASS` at `229b3ab`; frozen on port 4610. Candidate source at `f77599a`; shared streaming DTOs now live in a model module with cache compatibility re-exports. Pure candidate precedence, nearest-preview selection, and full-room retention planning sit behind the stable streaming façade, while a focused coordinator owns dynamic-overlay generation aborts and the exact capped 500/1000/2000/5000/10000 ms retry policy. All 35 streaming/preview suites pass 260 tests; the complete 177-file / 1,222-test gate, lint, TypeScript, generated bindings, production build, DOM contract, and diff checks are green. |
| T11 | World-streaming preparation and teardown lifecycles | Accepted: manual `PASS` at `848eda8`; frozen on port 4611. Candidate source at `516215d`; focused coordinators now own full-room preparation state, standard/portal activation ownership, cancellation/failure, deferred commits, phased teardown, forced destruction, retention, and reconciliation while the stable façade retains Phaser operations and their exact destruction order. All 36 streaming suites pass 264 tests; the complete 178-file / 1,226-test gate, lint, TypeScript, generated bindings, production build, DOM contract, and diff checks are green. |
| T12 | PvP combat geometry, ledger, presentation, and coordinator | Accepted: manual `PASS` at `212d54a`; frozen on port 4612. Candidate source at `419e6fd`; a two-client fake PartyKit baseline locks the 4.2-second countdown, combat relay, hit deduplication, 1.8-second invulnerability, death/finalization, and winner result. Pure geometry owns sword/gun/stomp envelopes and peer collision resolution; a combat coordinator owns the exact 180 ms hit throttle, 450 ms stomp cooldown, inbound ledger/rollback, hit IDs, action latch, and 25 ms instance cadence; local hearts/invulnerability/damage feedback now have a presentation owner. Invitation, setup, and match lifecycle remain in the arena controller. Focused 21-test coverage and the authoritative complete 182-file / 1,241-test rerun pass with lint, TypeScript, generated bindings, production build, DOM contract, and diff checks green. |
| T13 | Typed Pages Worker migration | Accepted: manual `PASS` at `68b362b`; frozen on port 4613 |
| T14 | Cloudflare Worker analysis/admin module split | Accepted: manual `PASS` at `4d024d3`; frozen on port 4614. Candidate source at `44c1698`; existing route/SQL/full-response contracts now guard suspicious analysis, launch statistics, room discovery, and difficulty voting. Suspicious scoring remains in its pure model; launch-stat range/time/summary normalization and D1 count/fallback reads now have separate model/store owners; difficulty vocabulary, cursors, predicates, and room/builder ordering now live in a pure model behind compatibility re-exports. Routes, SQL, cursor/cache behavior, PartyKit fail-open status, and JSON shapes are unchanged. Focused 47-test coverage and the complete 184-file / 1,247-test gate pass with lint, TypeScript, generated bindings, production build, DOM contract, Worker safety probes, and diff checks are green. |
| T15 | Thin admin UI entries and application modules | Accepted: manual `PASS` at `802ce11`; frozen on port 4615. Candidate source at `20319e1`; `launch-admin.html` and `suspicious-admin.html` retain their exact public entry URLs while the root scripts are now one-line compatibility entries. Application modules own the existing DOM lifecycles, a shared API client centralizes stable admin-key/header/error behavior, a reusable controller owns the exact visible-page 10-second polling cadence, and pure Launch/Suspicious view models own activity filters and queue derivation. HTML, DOM anchors, storage keys, native hash behavior, endpoints, rendered copy, and mutation flows are unchanged. Focused 17-test coverage and the complete 188-file / 1,255-test gate pass with lint, TypeScript, generated bindings, production build, DOM contract, and diff checks are green. |
| T16 | PartyKit protocol and server decomposition | Accepted: manual `PASS` at `d3ba27a`; frozen on port 4616. Candidate source at `6d0449a`; `PresenceServer` remains the hibernating lifecycle/routing shell while typed modules now own message/presence normalization, ghost/population accounting, construction-preview validation/storage/output, shard heartbeat normalization/aggregation, chat/invite relays, and PvP match state/normalization. Storage prefixes, exact 120-second preview/heartbeat boundaries, 80/250 ms batching, authorization, timer order, close behavior, and wire messages remain characterized. Focused 40-test coverage and the complete 193-file / 1,271-test gate pass with lint, TypeScript, generated bindings, production build, DOM contract, identity probe, and diff checks are green. A local PartyKit probe connected 48/48 signed clients and relayed 1,248 messages without bot errors. No PartyKit deployment occurred during QA. |
| T17 | Measured performance and dead-code closeout | Accepted: manual `PASS` at `2fe3548`; frozen on port 4617. Source commits `ec977da`, `1a54450`, and `5eb974f` reuse the movement-input snapshot, add opt-in round-robin controller profiling, and sever Phaser from three canvas-only render-entry import graphs. The comparable traversal profiler p95 is 7.4 ms; the repeat transition median is 19.5 ms with 5/5 correct crossings, zero seam hold, and no errors. Initial JS falls 75.3% for minted-room and 89.7-89.8% for the two renderer entries. Knip remains report-only because 2 unused-file candidates, 133 exports, 83 types, and other unexplained findings remain. |
