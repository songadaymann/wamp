...
| Tool | Description |
|------|-------------|
| **Draw** | Click or drag to stamp the current Tilesets selection (B). 1×1 stamps the selection as a group. 2×2–5×5 Square or Circle brushes sample occupied selected tiles per cell (old Shuffle). With a multi-tile 1×1 selection, optional Continuous Stamping (off by default) places a stamp on every hovered tile instead of snapping to the selection grid. Right-click erase follows the stamp or brush preview, including gaps. Ctrl/Cmd-click on the palette adds or removes individual tiles; a drag sets order 1…N and numbers overlay the selection |
...
| **Scramble** | Reorders existing tiles in a 1×1–5×5 window (V; default 3×3, last size remembered). Available only on Terrain with Advanced Tilesets. 1×1 Scramble still randomizes H and V because a single cell cannot reorder. Flip H / Flip V in the tileset menu cycle Off → flip (green) → Rand H / Rand V (orange) and apply to Draw, shapes, fill, and Scramble |
| **Eraser** | Click or drag to remove tiles. Right-click with Draw or Scramble uses that tool’s stamp or brush size |
