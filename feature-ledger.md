...
| --- | --- | --- | --- | --- | --- |
| XP / ratings / trust / badges progression | `needs-more-work` | Canonical: `feature/progression-xp-ratings-trust-badges` (local feature branch created April 4, 2026). Reference branch: none yet. | None recorded | The backend schema, progression service, rating APIs, profile progress surface, leaderboard metadata, capability gates, and first post-run modal pass are implemented locally on top of `main`, then moved onto the dedicated feature branch `feature/progression-xp-ratings-trust-badges`. Local D1 migration verification is fixed, the seeded course flow has a reusable authenticated smoke (`npm run smoke:progression-api:local`) that proves auth -> course clear -> rating -> profile progression -> leaderboard summary end to end, and the local profile/admin surfaces now include effective builder-cap visibility plus admin override tooling. The admin override path persists override columns on `user_progress`, exposes a search/save/clear console on `launch-admin.html`, and feeds the same server-side capability resolver that enforces placed-object / collectible / publish / claim caps. The remaining gap is broader room/course QA plus deciding how to handle the bad seeded room fixture (`0,-1` advertises `collect_target` but has no collectibles), not a blocker in the progression code itself. | Keep progression work on `feature/progression-xp-ratings-trust-badges`, do broader QA on real room/course content, and either repair or replace the seeded room smoke fixture before calling the feature ready. |
| Lighting system | `ready-for-merge` | Canonical: `feature/lighting` -> `ba7e971` (`safety/room-lighting-tuning-v2-2026-04-02`). Reference: `safety/room-lighting-retry-2026-04-01` (`6794a3c`), `safety/room-lighting-v1-2026-04-01` (`e1d771f`) | None recorded | Current team read is that lighting is effectively ready. The canonical branch name is now `feature/lighting`, but the code still sits on the older safety lineage because a straight cherry-pick onto current `main` was not clean. | Use `feature/lighting` as the merge-review branch. If/when landing to `main`, expect an integration pass rather than a trivial fast-forward. |
| Music system | `needs-more-work` | Canonical local continuation branch: `feature/music-sequencer` -> `1aa6d98`. Pushed safety branch: `origin/safety/main-based-room-sequencer-2026-04-02` (`1aa6d98`). Reference only: `safety/advanced-room-sequencer-2026-04-02` (`146f5a6`), `safety/advanced-room-sequencer-followup-2026-04-02` (`acc9a9c`), `safety/advanced-room-sequencer-save-recovery-2026-04-02` (`8ec87e6`), `safety/room-music-followup-snapshot-2026-04-01` (`37c1687`) | None recorded | Current team read is that music is directionally correct but still needs more work. `feature/music-sequencer` is the branch name to continue on locally, and `origin/safety/main-based-room-sequencer-2026-04-02` is the pushed remote branch that matches that commit. | Continue new music work on `feature/music-sequencer` and cherry-pick only truly music-scoped follow-ups into it. |
