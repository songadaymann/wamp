...
| Wall-jump feel follow-up | `needs-more-work` | Canonical: `wip/wall-jump-feel-2026-04-04`. Reference: `fix/wall-jump-chain-2026-03-29` (`becdedd`) | `https://safety-wall-jump-feel-2026-0.wampland.pages.dev` | Clean April 4 continuation branch now exists off `main`. Current pass restored chain-aware wall contact, separated wall contact from slide-only descent behavior, widened airborne wall buffering, and added movement debug output for wall/buffer state. Live probes during this pass did hit wall-slide / wall-jump states, but the exact buffered-contact feel still needs QA in a tighter wall-jump room because the current preview harness is real-time rather than deterministic. A deployed Pages preview now exists for manual feel testing; the stock readonly smoke on that preview only failed at the editor-permission step, while a lighter boot/play probe passed. | Continue wall-jump tuning on `wip/wall-jump-feel-2026-04-04`, preferably with a purpose-built shaft/test room or a more deterministic step hook before deciding whether the `240ms` wall buffer is final. |
