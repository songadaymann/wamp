...
| Feature | Status | Canonical / Reference Branches | Preview Deploy | Notes | Next Clean Action |
| --- | --- | --- | --- | --- | --- |
| Overworld zoom tilemap collision fix | `testing` | `fix/overworld-zoom-tilemap-collision-2026-04-06` | Branch alias: `https://safety-overworld-zoom-tilema.wampland.pages.dev` | Narrow main-based gameplay/runtime fix. Root cause: live objects in still-loaded rooms kept Arcade colliders pointing at a room `terrainLayer` after world streaming unloaded that room during zoom-driven LOD changes, which caused Phaser `collideSpriteVsTilemapLayer` to read `layerData.tileWidth` from a destroyed tilemap layer. The fix adds a streaming-level callback that rebuilds remaining live-object world colliders whenever a full room unloads or is invalidated. Safety Worker deploy for this QA pass is `https://everybodys-platformer-safety.novox-robot.workers.dev` at version `41d0467b-1927-4913-bcea-a9b41ec5393c`. Repo preview smoke reached boot + play and only failed on a minted-room edit-button gate, while the targeted deployed-preview zoom repro completed with no relevant runtime errors. | QA the safety preview specifically for overworld play/browse zoom transitions, then merge this branch as a standalone hotfix if the crash is confirmed resolved. |
