...
async function enterPlayableRoom(page, roomId) {
  ...
  await page.waitForFunction((requestedRoomId) => {
    try {
      const state = JSON.parse(window.render_game_to_text?.() ?? '{}')?.activeScene;
      if (state?.selectedState !== 'published') return true;
      return state?.leaderboards?.room?.roomId === requestedRoomId;
    } catch {
      return false;
    }
  }, selection.roomId ?? roomId, { timeout: 120_000 });
...
async function run() {
  ...
  if (options.scenario === 'idle') {
    ...
    await pinPlayerToBenchmarkRoom(page, transition.position);
    await page.waitForTimeout(options.durationMs);
  } else {
