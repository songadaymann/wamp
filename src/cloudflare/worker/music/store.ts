...
async function loadLatestMusicPhraseSummary(
  ...
  const row = await env.DB.prepare(
    ...
    .first<{ ordinal: number | string | null; fingerprint: string | null }>();

  if (!row || typeof row.fingerprint !== 'string') {
    return null;
  }
...
export async function prepareMusicPhrasePublishStatements(
  ...
  for (const instrumentId of ROOM_PATTERN_INSTRUMENT_IDS) {
    ...
    const ordinal = latestPhrase ? latestPhrase.ordinal + 1 : 0;
