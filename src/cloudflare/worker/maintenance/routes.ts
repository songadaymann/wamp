...
export async function handleTestReset(request: Request, env: Env): Promise<Response> {
  ...
  const counts = {
    ...
    chatAdmins: await countRows(env, 'chat_admins'),
    chatBans: await countRows(env, 'chat_bans'),
  ...
  await env.DB.batch([
    ...
    env.DB.prepare('DELETE FROM chat_bans'),
    env.DB.prepare('DELETE FROM chat_admins'),
