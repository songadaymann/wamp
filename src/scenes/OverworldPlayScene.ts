...
export class OverworldPlayScene extends Phaser.Scene {
  ...
  create(data?: OverworldPlaySceneData): void {
    ...
    this.hudBridge = new OverworldHudBridge({
      onPlayersOnlineEntryJoin: (entry) => this.joinOnlineRosterEntry(entry),
    });
  ...
  private async joinOnlineRosterEntry(entry: OverworldOnlineRosterViewEntry): Promise<void> {
    await this.jumpToCoordinates(entry.roomCoordinates);
