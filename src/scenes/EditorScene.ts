...
export class EditorScene extends Phaser.Scene {
  ...
  private musicEditorTab: EditorMusicTab = 'advanced';
  private musicPickerLaneId: RoomMusicLaneId | null = null;
  private musicPickerBarIndex: number | null = null;
  private musicPreviewState: 'stopped' | 'playing' | 'paused' = 'stopped';
  private musicPickerPreviewClipId: string | null = null;
  ...
  private readonly handleDocumentKeyDown = (event: KeyboardEvent): void => {
    ...
    if (key === 'escape') {
      ...
      if (this.musicPickerLaneId) {
        this.closeRoomMusicPicker();
        return;
      }
  ...
  private readonly handleShutdown = (): void => {
    window.removeEventListener(BACKGROUND_CHANGED_EVENT, this.handleBackgroundChanged);
  ...
  constructor() {
    ...
    this.editRuntime = new EditorEditRuntime(this, {
      ...
      updateBackgroundControls: (backgroundValue) => {
        syncBackgroundControls(document, backgroundValue);
      },
    ...
    this.backgroundController = new EditorBackgroundController(this, this.worldRepository, {
      ...
      getIgnoredBackgroundObjects: () => {
        ...
        const overlays = [
          ...
          this.borderGraphics,
          ...this.musicPatternController.getIgnoredObjects(),
        ];
  ...
  create(data?: EditorSceneData): void {
    ...
    window.addEventListener(BACKGROUND_CHANGED_EVENT, this.handleBackgroundChanged);
  ...
  private resetRuntimeState(): void {
    ...
    this.musicEditorTab = 'advanced';
    this.musicPickerLaneId = null;
    this.musicPickerBarIndex = null;
    this.musicPreviewState = 'stopped';
    this.stopRoomMusicClipPreview();
  ...
  private renderInspectorUi(): void {
    ...
    if (
      ...
      const defaultContentsLabel = this.getDefaultContainerContentsLabel(focusedContainer.id);
      const state: EditorInspectorState = {
        ...hiddenState,
        visible: true,
        ...
        containerClearTitle: focusedContainer.containedObjectId
          ? ''
          : defaultContentsLabel
            ? `${this.getContainerName(focusedContainer.id)} is already using its default ${defaultContentsLabel}.`
  ...
  private handleObjectPlace(pointer: Phaser.Input.Pointer): void {
    ...
    if (placed && canPlacedObjectTriggerOtherObjects(placed)) {
      ...
      const defaultContentsLabel = this.getDefaultContainerContentsLabel(placed.id);
      this.containerStatusText = defaultContentsLabel
        ? `${this.getContainerName(placed.id)} placed. It will pop ${defaultContentsLabel} unless you assign a different collectible.`
        : `${this.getContainerName(placed.id)} placed. Select a ${this.getContainerAcceptedContentsLabel(placed.id)} and click it to fill the container.`;
      this.renderContainerContentsPanel();
    }
  }
  ...
  clearFocusedContainerContents(): void {
    ...
    if (this.editRuntime.setContainerContents(focused.instanceId, null)) {
      ...
      const defaultContentsLabel = this.getDefaultContainerContentsLabel(focused.id);
      this.containerStatusText = defaultContentsLabel
        ? `${this.getContainerName(focused.id)} will now pop ${defaultContentsLabel} by default.`
        : `${this.getContainerName(focused.id)} is now empty.`;
      this.renderContainerContentsPanel();
    }
  }
  ...
  private getDefaultContainerContentsLabel(objectId: string): string | null {
    const defaultObjectId = getDefaultContainerObjectId(objectId);
    return defaultObjectId ? getObjectById(defaultObjectId)?.name ?? null : null;
  }
  ...
  private stopRoomMusicClipPreview(): void {
    globalRoomMusicController.stopPreviewClip();
    this.musicPickerPreviewClipId = null;
  }
  ...
  openRoomMusicPicker(laneId: RoomMusicLaneId, barIndex: number): void {
    this.musicPickerLaneId = laneId;
    this.musicPickerBarIndex = barIndex;
    this.renderEditorUi();
  }
  ...
  assignRoomMusicBarClip(laneId: RoomMusicLaneId, barIndex: number, clipId: string): void {
    this.editRuntime.setRoomMusicLaneBarClip(laneId, barIndex, clipId);
    this.stopRoomMusicClipPreview();
    this.musicPickerLaneId = null;
    this.musicPickerBarIndex = null;
    if (this.musicPreviewState === 'playing') {
  ...
  clearRoomMusicBarClip(laneId: RoomMusicLaneId, barIndex: number): void {
    this.editRuntime.setRoomMusicLaneBarClip(laneId, barIndex, null);
    this.stopRoomMusicClipPreview();
    this.musicPickerLaneId = null;
    this.musicPickerBarIndex = null;
    if (this.musicPreviewState === 'playing') {
  ...
  previewRoomMusicClip(clipId: string): void {
    if (this.musicPickerPreviewClipId === clipId) {
      this.stopRoomMusicClipPreview();
      this.renderEditorUi();
      return;
    }

    this.musicPickerPreviewClipId = clipId;
    void globalRoomMusicController.previewClip('wamp-v1', clipId).finally(() => {
      if (this.musicPickerPreviewClipId === clipId) {
        this.renderEditorUi();
      }
    });
  ...
  private renderEditorUi(): void {
    ...
    this.uiBridge?.render(
      buildEditorUiViewModel({
        ...
        courseEditorState,
        roomMusic: this.roomMusic,
        musicModeActive: this.musicModeActive,
        musicPreviewState: this.musicPreviewState,
        musicPatternInstrumentTab: this.musicPatternController.getActiveInstrumentTab(),
      }),
    );
    this.renderInspectorUi();
  }
  ...
  describeState(): Record<string, unknown> {
    const mainCamera = this.cameras?.main ?? null;
    ...
    return {
      ...
      music: {
        modeActive: this.musicModeActive,
        editorTab: this.musicEditorTab,
        instrumentTab: this.musicPatternController.getActiveInstrumentTab(),
        legacyStemNoticeVisible: this.musicPatternController.getLegacyStemNoticeVisible(),
        pastePreviewActive: this.musicPatternController.isPastePreviewActive(),
        previewState: this.musicPreviewState,
        pickerLaneId: this.musicPickerLaneId,
        pickerBarIndex: this.musicPickerBarIndex,
        pickerPreviewClipId: this.musicPickerPreviewClipId,
        roomMusic: this.roomMusic,
        playback: globalRoomMusicController.getDebugState(),
      },
      canSaveDraft: this.roomPermissions.canSaveDraft,
      ...
      camera: mainCamera
        ? {
            scrollX: Math.round(mainCamera.scrollX),
            scrollY: Math.round(mainCamera.scrollY),
          }
        : null,
