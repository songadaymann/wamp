...
function syncPolling(): void {
  ...
  pollingController.sync(shouldPoll);
}
...
async function refreshSnapshot(force = false): Promise<void> {
  ...
  try {
    const response = await adminResponseClient.request('/api/admin/launch-stats', {
      headers: {
...
async function refreshGameJams(): Promise<void> {
  ...
  renderGameJams();
  try {
    const response = await adminResponseClient.request('/api/admin/game-jams', {
      headers: {
        'x-admin-key': adminKey,
      },
    });
...
async function searchProgressionUsers(): Promise<void> {
  ...
  try {
    const response = await adminResponseClient.request(
...
async function loadProgressionUser(userId: string): Promise<void> {
  ...
  try {
    const response = await adminResponseClient.request(`/api/admin/progression/users/${encodeURIComponent(userId)}/caps`, {
      headers: {
        'x-admin-key': adminKey,
      },
    });
...
async function saveProgressionOverride(clear: boolean): Promise<void> {
  ...
  try {
    const response = await adminResponseClient.request(
      `/api/admin/progression/users/${encodeURIComponent(selectedProgressionUser.userId)}/caps`,
      {
        method: 'POST',
        headers: {
...
async function refreshRoomComments(): Promise<void> {
  ...
  try {
    const response = await adminResponseClient.request(
      `/api/admin/room-comments?status=${encodeURIComponent(status)}&limit=80`,
      {
        headers: {
          'x-admin-key': adminKey,
        },
...
async function reviewRoomComment(
  ...
  try {
    const response = await adminResponseClient.request(
      `/api/admin/room-comments/${encodeURIComponent(commentId)}/review`,
      {
        method: 'POST',
        headers: {
          'content-type': 'application/json',
