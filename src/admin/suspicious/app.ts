...
function renderQueueTabs(): void {
  ...
  const tabs: Array<{ id: SuspiciousQueueTab; label: string }> = [
...
function getVisibleUsers(): SuspiciousUserCase[] {
  return filterSuspiciousUsers(state.users, state.queueTab);
}

function getQueueTabCounts(): Record<SuspiciousQueueTab, number> {
  return countSuspiciousQueueTabs(state.users);
}

function identityChipClass(user: SuspiciousUserCase): SuspiciousSeverity {
  return getSuspiciousIdentityTone(user);
}
