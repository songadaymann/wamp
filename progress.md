...
- Room music follow-up on April 1, 2026:
  ...
  - stopping room music from editor preview or when leaving world play now uses a much faster immediate fade (`0.08s`), while room-to-room play transitions still stay bar-aligned
  - the music overlay now locks the rest of the editor shell while open, and phone nav is forced to the `Music` sheet so the overlay cannot get stranded behind another editor panel
  - the arrange UI is now a true bar grid: each lane exposes 4 bar cells, the picker operates on `lane + bar`, and clear/apply actions only affect that bar
  - `npm run build` passed after the model/runtime/editor/mobile updates
  - browser follow-up still worth doing: open the editor on phone layout, enter music mode, switch bars, then confirm other editor tabs stay unavailable until `Close Music`
...
- Profile ownership + online roster join follow-up on April 3, 2026:
  - tightened profile room sourcing so profile `Published Rooms` now lists only rooms claimed by that user and currently published; last-editor fallback no longer makes tiny edits show up on someone else’s profile
  - updated profile UI copy/stat labels to match that ownership rule (`Published Rooms`, `No claimed published rooms yet`, `Published rooms`)
  - added direct join behavior to the bottom-bar online roster: non-self roster rows are now clickable buttons that close the popover and join that player’s room
  - the join action is wired through `OverworldHudBridge` via a scene callback instead of making the HUD bridge reach into scene state directly
  - local verification:
    - `npm run build` passed
    - required skill-client smoke ran at `output/web-game/online-roster-join-smoke/`
    - targeted two-context Playwright check at `output/web-game/online-roster-join-check/summary.json`
    - `popover-before-join.png` shows the live roster entry for `Guest wi36` in `Room -1,0`
    - `after-join.png` shows the second client in play mode in `Room -1,0` with the remote ghost visible and no captured console errors

- Solid color room backgrounds on April 2, 2026:
  - added a small `src/backgrounds/model.ts` helper so room backgrounds can stay a single persisted string while now supporting either a named parallax background or a normalized `solid:#RRGGBB` value
  - room snapshot cloning / normalization, mint payload build + decode, worker command validation, and token metadata labels now all understand solid-color backgrounds without needing a new room schema field
  - updated every room background renderer path to honor solid colors:
    - editor preview backgrounds in `src/scenes/editor/backgrounds.ts`
    - play-mode room backgrounds in `src/scenes/overworld/worldStreaming.ts`
    - snapshot / chunk / minted canvas renders in `src/visuals/roomSnapshotTexture.ts` and `src/mint/roomMetadataRender.ts`
  - editor UI now includes a `Solid Color` option in the shared Background section plus a live color picker that:
    - appears only when `Solid Color` is active
    - previews changes immediately in the room
    - preserves the last chosen solid color when toggling back to other background types
  - verification:
    - `npm run build` passed
    - generic Playwright client smoke artifacts are in `output/web-game/background-solid-color-client-smoke/`
      - the headless full-page shot still mostly shows the boot overlay in this repo, but the state capture completed
    - targeted editor validation artifacts are in `output/web-game/background-solid-color-check/`
      - `summary.json` confirms `scene: "editor"`, `stateBackground: "solid:#ff6600"`, `selectValue: "solid_color"`, visible color controls, and active solid-color card state
      - `editor-solid-color-visible.png` shows the room canvas filled with the selected solid color and the background picker switched to `Solid Color`

- Compact music dock follow-up on April 2, 2026:
  - shrank the sequencer music chrome from a large header modal into a compact top dock that sits over the room as one narrow strip
  - instrument tabs now render as rounded top tabs in the dock, with transport, pitch, and octave controls grouped inline beside them instead of consuming a full-width header block
  - changed preview transport to explicit `Play` / `Stop` controls in the music UI and keyboard handling:
    - `Play` no longer flips its label to `Pause`
    - the spacebar music shortcut now toggles `play` vs `stop`, not `play` vs `pause`
    - the UI view-model now exposes `playDisabled` separately from `stopDisabled`
  - updated the editor music control wiring so the preview button always issues `play`, while stop remains its own action
  - verification:
    - `npx tsc --noEmit` passed
    - `npm run build` passed
    - lightweight Playwright client artifact: `output/web-game/music-compact-dock-client-check/`
    - follow-up headless editor-entry probes are still noisy in this repo state because local PartyKit is offline and the selected room can surface as a locked minted room, so the compact dock UI was not fully validated through a clean world-button click path in this pass

- Music stop/reset follow-up on April 2, 2026:
  - fixed manual `Stop` so it resets the shared room-music transport instead of only fading the current playback out
  - fresh immediate `Play` now anchors transport start time to the new playback start, so editor preview and world play restart from beat one instead of inheriting the old loop offset
  - only explicit stop paths reset transport; bar-aligned room-to-room transitions and silence transitions still preserve shared musical continuity
  - verification:
    - `npx tsc --noEmit` passed
    - `npm run build` passed
    - headless transport probe was still noisy because direct editor entry is flaky in the current local repo state, but the transport reset itself is a narrow controller change and compiled cleanly

- Advanced sequencer follow-up on April 2, 2026:
  - rebalanced the pattern sequencer mix in `src/music/patternRenderer.ts` by splitting tonal vs drum buses, nudging the tonal instruments up slightly, and pulling the drum bus down so kicks/snares stop dominating the loop
  - changed sequencer editing in `src/scenes/editor/musicPatternEditor.ts` so a normal click now toggles the targeted square:
    - clicking an empty square places a note/hit
    - clicking the same occupied square removes it
    - drag now continues in the inferred add/remove mode from the initial click
  - updated the music overlay hint copy in `src/scenes/editor/viewModel.ts` and `index.html` so it matches the new toggle behavior instead of referring to draw/erase as the primary interaction
  - verification:
    - `npx tsc --noEmit` passed
    - `npm run build` passed
    - browser smoke artifacts are in `output/web-game/music-toggle-balance-smoke/`
    - direct toggle validation artifact is in `output/web-game/music-toggle-balance-check/toggle-state.json`
      - first interaction created a triangle note at step `0`, row `10`
      - second interaction cleared the same square back to `null`

- Advanced room-grid sequencer on April 2, 2026:
  - promoted the planned `Advanced` music surface into the primary room-music authoring flow and kept legacy stem-arrangement playback/persistence backward-compatible under a `RoomMusic` union
  - added a dedicated pattern-music bundle under `src/music/`:
    - `pattern.ts` owns pattern types, defaults, row mapping, drum lane metadata, normalization, and timing constants
    - `patternKit.ts` synthesizes the repo-owned drum one-shots in code so v1 does not depend on uncertain third-party sample licensing
    - `patternRenderer.ts` renders `pattern` room music into loopable `AudioBuffer`s using triangle / saw / square synthesis plus the generated drum kit
  - extended `src/music/controller.ts` so room playback now handles both legacy stem arrangements and new pattern loops on the same transport, including pattern/stem crossfades and the existing short stop fade
  - replaced the old arrange/advanced music UI split with the sequencer-first shell:
    - the room canvas itself is now the sequencer grid with `32` active steps and `8` dimmed margin columns
    - beat dividers are thicker every `4` columns with a stronger midpoint at step `16`
    - top controls switch between `Drums`, `Triangle`, `Saw`, and `Square`
    - pitch mode toggles between `Scale` and `Chromatic`
    - octave shift is per tonal tab and clamped to `-2..+2`
  - added `src/scenes/editor/musicPatternEditor.ts` as the editor-side sequencer controller instead of folding more music logic into the large scene file:
    - draws the overlay, playhead, labels, disabled drum rows, and paste previews
    - reuses editor tools for `Draw`, `Erase`, and `Copy`
    - supports monophonic tonal steps, drum stacking, rectangular copy, paste preview, and undo/redo through the normal room history path
  - persistence/runtime/editor wiring now understands `kind: 'pattern'` end-to-end:
    - room normalization, cloning, empty checks, room history, and edit-runtime dirty tracking support both `stemArrangement` and `pattern`
    - existing stem rooms still remain playable and can be explicitly replaced with a pattern later
  - fixed one validation-found regression while wiring the probes:
    - `EditorScene.describeState()` now guards the camera reference so debug/state probes do not throw during transient scene teardown
  - verification:
    - `npx tsc --noEmit` passed
    - `npm run build` passed
    - Playwright client smoke artifacts: `output/web-game/advanced-room-sequencer-smoke/` and `output/web-game/advanced-room-sequencer-smoke-postfix/`
    - direct editor-state validation artifact: `output/web-game/advanced-room-sequencer-direct-check/desktop-direct-validation.json`
      - confirmed music mode locks the editor shell via `data-editor-music-mode='true'` and `data-editor-music-ui-locked='true'`
      - confirmed pattern room music is created as `kind: 'pattern'`
      - confirmed tonal edit/copy/paste data persisted at steps `0-1` and `4-5`
      - confirmed drum hits persisted at steps `0`, `4`, and pasted `8`
      - confirmed pitch mode switched to `chromatic`
      - confirmed triangle octave shift moved from `-1` default to `0`
      - confirmed undo history remained available after the pattern edits
    - automated mobile-specific validation is still incomplete:
      - the headless mobile browser path did not reach a stable app-ready + editor-active state consistently enough to trust as evidence
      - code paths for forced `Music` sheet lock and disabled non-music tabs are in `src/ui/mobile/controller.ts` and the matching CSS lock rules are in `src/styles/sections/editor.css`
...
- Desktop world HUD play-mode collapse on April 2, 2026:
  ...
  - verification:
    ...
    - `output/web-game/world-hud-play-collapse-smoke/state-0.json` captured the refreshed real-app smoke after the change

- Local frontend dev port moved on April 1, 2026:
  - changed the Vite dev server and npm frontend scripts from `3000` to `3232`
  - updated the local worker `APP_BASE_URL` defaults to point at `http://localhost:3232` / `http://127.0.0.1:3232` so local auth links still target the running frontend
  - verification:
    - `npm run build` passed after the port/config update
...
- 2026-04-02: Tightened the advanced room sequencer UI by turning preview into a single Play/Stop toggle and adding per-instrument right-margin mix controls.
  - The music overlay transport now uses one button that flips between `Play` and `Stop`; explicit stop resets the preview transport so replay starts from beat one.
  - Added persistent per-instrument `volume` and `pan` settings to pattern room music and wired the renderer to stereo output so the sequencer margin controls affect playback directly.
  - Added a compact mix panel in the unused right-side room margin with a horizontal pan control above a vertical volume slider for the active instrument tab.
  - Verification:
    - `npx tsc --noEmit` passed
    - `npm run build` passed
    - browser probing was only partially conclusive because local editor automation is still noisy in this repo; direct page checks confirmed the editor shell but did not give a trustworthy full sequencer UI assertion
- 2026-04-02: Rebalanced the advanced sequencer voices and split tonal articulation so drag creates ties while click-by-click entry stays discrete.
  - tonal tracks now persist per-step tie flags, old saved patterns normalize with legacy ties preserved, and the renderer/canvas overlay only merge adjacent steps when the later step is explicitly tied
  - sequencer input now keeps single clicks discrete and only creates a tie when the user drags across adjacent cells on the same pitch; tonal copy/paste now carries those tie boundaries too
  - pattern playback balance now pulls saw/square down, gives triangle/drums more weight, and adds light soft-drive shaping to triangle and drums for more body without forcing builders to trim every part manually
  - verified the room music save/publish path is already wired through room snapshot export and repository publish/save clones, so no extra persistence patch was needed
  - Verification:
    - `npx tsc --noEmit` passed
    - `npm run build` passed
    - Playwright client wrote `output/web-game/music-articulation-smoke/state-0.json` and `output/web-game/music-articulation-smoke/shot-0.png`; the screenshot still came back black and `appFeedback.ready` stayed false, so runtime browser validation remains noisy in this environment
