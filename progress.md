...
- Lighting section nesting pass on April 2, 2026:
  - moved the Lighting controls out of their standalone sidebar section and nested them directly inside the existing `Advanced` section
  - kept the same control ids and behavior, adding only a small subsection divider so the nested block still scans cleanly
  - verification:
    - `npm run build` passed
    - required `develop-web-game` client run wrote:
      - `output/web-game/lighting-inside-advanced-client/shot-0.png`
      - `output/web-game/lighting-inside-advanced-client/state-0.json`
    - targeted editor captures wrote:
      - `output/web-game/lighting-inside-advanced-check/summary.json`
      - `output/web-game/lighting-inside-advanced-check/editor-expanded.png`
    - targeted probe confirms:
      - `lightingInsideAdvanced: true`
